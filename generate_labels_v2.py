...
class tagging():
    def show_topk_sliding_window(classes, duration,min_duration,target_sr, m2d, wav_file, k=20, hop=1,noise_factor=2):
...
if __name__=="__main__":
    ...
    project_path = "./"
    files_per_batch = 5
    meta_dir = "./sound_datasets/rare_sound_event/meta"
    output_path = './event/noise/'
    ...
    for i in range(1):
        ...
        random_int = 1
        files_babycry = list(Path('./sound_datasets/rare_sound_event/eval/audio/babycry').glob('*.wav'))[random_int]
        files_glassbreak = list(Path('./sound_datasets/rare_sound_event/eval/audio/glassbreak').glob('*.wav'))[random_int]
        files_gunshot = list(Path('./sound_datasets/rare_sound_event/eval/audio/gunshot').glob('*.wav'))[random_int]
        files = [*files_babycry]
        ...
        df_representation.to_csv('representation_orig_'+str(random_int)+'.csv')
        ...
        df_representation.to_csv(output_path+model_event_name+'_representation_result_'+str(random_int)+'.csv')
    ...
    matrix_total.to_csv(output_path+model_event_name+'_matrix_total_'+classification_type+'_'+event+'_'+timesequence+'.csv')
