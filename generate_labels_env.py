...
class tagging():
    ...
    def show_topk_for_all_frames(classes,target_sr, m2d, wav_file, k=10):
        #print(wav_file)
        print(m2d.cfg.sample_rate)
        # Loads and shows an audio clip.
        wav, sr = librosa.load(wav_file, mono=True, sr=44100)
...
class utils():
    ...
    def rank(score_total,wav):
        ...
        df_score.to_csv(output_path+'score_'+wav+'.csv')
        return df_score
    ...
    def envFiltering_simple(df,wav,k1=5,k2=3):
        ...
        df_filtering_simple.to_csv(output_path+'df_filtering_simple_'+str(wav)+'.csv')
        ...
        df_filtering_simple_transform.to_csv(output_path+'df_filtering_simple_trasnform_'+str(wav)+'.csv')
    ...
    def envFiltering_weighted(df,wav,k=5):
        score=[]
        env=[]
        wav_names=[]
        print(df.columns)
        ...
        df_filtering_weighted.to_csv(output_path+'df_filtering_weighted_'+str(wav)+'.csv')
        ...
        df_filtering_weighted_transform.to_csv(output_path+'df_filtering_weighted_trasnform_'+str(wav)+'.csv')
    ...
    def envExtract(df_wav_total,wav,envnt_list,k=5):
        #ipca clustering and define
        #print(df_wav_total.columns)
        df_wav_total=df_wav_total.drop(['level_0','index'],axis=1)
        df_wav_total=df_wav_total.reset_index()
        env_extracted=[]
        score_extracted=[]
        classes_=[]
        class_extracted=[]
        for i in range(len(df_wav_total)):
            #print(df_wav_total['classes_level0'][i])
            if type(df_wav_total['classes_level0'][i])==list:
                classes_=df_wav_total['classes_level0'][i]+classes_
            elif type(df_wav_total['classes_level0'][i])==str:
                classes_.append(df_wav_total['classes_level0'][i])
        for env in envnt_list:
            classes_ = utils.dataClean(classes_)
            env = utils.dataClean(env)
            similarity_env=utils.similarity(classes_,env,model_environment)
        ...
        df_level1.to_csv(output_path+'df_level1_trasnform_'+str(wav)+'.csv')
...
class evaluation():
    def evaluationMatrix(df_representation,classification_type,event):
        ...
        auc_value=roc_auc_score(actual_event,predict_event,average='micro')
        pauc_value=roc_auc_score(actual_event,predict_event,average='micro',max_fpr=0.1)
    ...
    def visualize(matrix_total,actual_event_total,predict_event_total):
        ...
        geom_point(aes(x="value",
                   ...
                   alpha=0.5, 
                   color="black") + \
        labs(title="Distribution of Actual/Predicted Event: "+event) + \
        xlab("Value") + \
        ylab("Density"))
        ...
        plt.title('ROC_AUC Curve of Event: '+event)
        plt.legend(loc='best')
        fig=plt.gcf()
        ...
        plt.title('Precision_Recall Curve of Event: '+event)
        plt.legend(loc='best')
        fig=plt.gcf()
        ...
        fig,ax=plt.subplots()
        plt.title('Accuracy of Prediced Event: '+event)
...
if __name__=="__main__":
    project_path = "./"
    output_path = '/Users/sharontan6217/Documents/m2d_sed/env/no_noise/'
    files_per_batch = 5
    meta_path = "/Users/sharontan6217/Documents/m2d_sed/sound_datasets/rare_sound_event/meta"
    timesequence = str(datetime.now())[-6:]
    classification_type='environment'
    event='gunshot'
    if classification_type=='event':
        if event=='gunshot' or event=='glassbreak':
            min_duration = 48000
            duration = 2.
            target_sr=16000
        elif event=='babycry' :
    ...
    for i in range(10):
        ...
        #df_representation_total = pd.concat([df_representation_total,df_representation],axis=0)
        ...
        '''
    '''
    df_representation_total = df_representation_total.drop(['level_0','index'],axis=1)
    df_representation_total = df_representation_total.reset_index()
    df_representation_total.to_csv('representation_result_'+str(random_int)+'.csv')
    
    matrix_total.to_csv('matrix_total_'+classification_type+'_'+event+'_'+timesequence+'.csv')
    ...
    output.to_csv('output_'+classification_type+'_'+event+'_'+timesequence+'.csv')
